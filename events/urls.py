from django.urls import path
from . import views

urlpatterns = [
    # Event Category URLs
    path('i/categories/', views.EventCategoryList, name='event_category_list'),
    path('i/categories/create/', views.CategoryCreateView.as_view(), name='event_category_create'),
    path('i/categories/<int:pk>/edit/', views.CategoryEditView.as_view(), name='event_category_edit'),
    
    # Event Subcategory URLs
    path('i/subcategories/', views.EventSubCategoryList, name='event_sub_category_list'),
    path('i/subcategories/create/', views.SubCategoryCreateView.as_view(), name='event_sub_category_create'),
    path('i/subcategories/<int:pk>/edit/', views.SubCategoryEditView.as_view(), name='event_sub_category_edit'),

    path('i/list/', views.EventList, name='event_list'),
    path('i/create/', views.EventCreateView.as_view(), name='event_create'),
    path('i/<int:pk>/edit/', views.EventEditView.as_view(), name='event_edit'),

    path('i/media-info/list/', views.EventMediaInfoList, name='event_media_info_list'),
    path('i/media-info/details/', views.EventMediaInfoDetails, name='event_media_info_details'),
    path('i/media/list/', views.EventMediaList, name='event_media_list'),
    path('i/media/create/', views.EventMediaCreate.as_view(), name='event_media_create'),
]